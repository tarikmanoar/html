jQuery(document).ready(function($) {
	...
	$('.mtp-layout-select').change(function(){
		if ( $(this).val() == 'boxed-layout' ) {
			var contentWidth = $('.mtp-layout-width-select').val();
	...
	// Header style
	$('.mtp-header-select').change(function(){
		if ($('.mtp-layout-select').val() != 'full-width-layout'){
			alert('Please select "Full width" layout!')
		}
		if ( $(this).val() == 'fixed-header' ) {
			var contentWidth = $('.mtp-layout-width-select').val();
			$('#site-header #header-container, #top-bar').css('max-width', contentWidth + 'px');
			$('#site-header').addClass('fixed-width');
		} else {
			$('#site-header #header-container, #top-bar').css('max-width', '100%');
			$('#site-header').removeClass('fixed-width');
		}
	});
	
	// Page: Contained heder fix
	if ( $('body').hasClass('page-id-5737') ) {
		$('.mtp-header-select>option:eq(1)').prop('selected', true);
		$('.mtp-header-select').prop('disabled', true);
	}
